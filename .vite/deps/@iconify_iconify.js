...
function removeRootNode(root) {
  nodes = nodes.filter(
    (node) => root !== node && root !== (typeof node.node === 'function' ? node.node() : node.node)
  )
}
...
var booleanAttributes = ['inline', 'hFlip', 'vFlip']
  ...
  function getIcon2(icon, load) {
    ...
    if (load && !isPending(icon)) {
      const providerIconsToLoad =
        iconsToLoad[provider] || (iconsToLoad[provider] = /* @__PURE__ */ Object.create(null))
      const set =
        providerIconsToLoad[prefix] || (providerIconsToLoad[prefix] = /* @__PURE__ */ new Set())
      set.add(name)
    }
    return {
      status: 'loading'
