...
var Options = class {
  ...
  setRankedDictionaries() {
    ...
    Object.keys(this.dictionary).forEach((name) => {
      ...
      rankedDictionariesMaxWorkSize[name] = this.getRankedDictionariesMaxWordSize(
      ...
      if (cur.isTerminal()) {
        ...
        for (const sub of subs) {
          ...
          this.helper({
            ...
            consecutiveSubCount:
              lastSubLetter === cur.parents.join('') ? consecutiveSubCount + 1 : 1
          })
          this.buffer.pop()
          if (this.finalPasswords.length >= this.limit) {
            return
...
var dictionaryMatcher = ({ rank, reversed, l33t, subs, token, dictionaryName }) => {
...
var estimatePossiblePatterns = ({ token, graph, turns }) => {
...
var MatchSpatial = class {
  ...
  checkIfShifted(graphName, password, index) {
    if (
      !graphName.includes('keypad') && // initial character is shifted
...
export { Options, debounce, zxcvbn, zxcvbnAsync, zxcvbnOptions }
